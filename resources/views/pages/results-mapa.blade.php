...
@section ('displayResults')
    <div class="row d-xl-none" style="margin-bottom: 20px">
        ...
        @endhandheld
            ...
            <div class="row">
                <div class="col-8"><h1 class="pb-2" style="display: inline; font-size: 24px">@if(isset($request->region) && (ucfirst($request->region) == 'Zakopane' || ucfirst($request->region) == 'Kościelisko' || ucfirst($request->region) == 'Witów')){{ $request->region }}@endif<span class="d-xl-none">@if(!$request->has('complex-only'))({{ $countedApartaments }}) @else() ({{ $countedObjects }}) @endif</span></h1><span class="pb-2 d-none d-xl-inline">@if(!$request->has('complex-only')) ({{ $countedApartaments }} {{trans_choice('messages.apartaments', $countedApartaments)}}) @else() ({{ $countedObjects }} {{trans_choice('messages.objects3', $countedObjects)}}) @endif</span></div>
